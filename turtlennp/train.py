...
from mymodel_subsel import model
from normalize import normalize

device = torch.device("cuda")
logfile = "out.log"

#atoms = read("peroxide.xyz")
atoms = read("diels.xyz")
subsel = torch.arange(22, device = device)
#subsel = torch.arange(len(atoms), device = device)
#typemap = {el:i for i,el in enumerate(np.unique(atoms.get_chemical_symbols()))}
typemap = {'H':0,'C':1,'O':2}
at = torch.tensor([typemap[i] for i in atoms.get_chemical_symbols()], device = device)

#xyz_arr = torch.tensor(np.load("gromacs/mol.acpype/positions.npy"), requires_grad = True, device=device)
#frc = torch.tensor(np.load("gromacs/mol.acpype/forces.npy"), device=device)*0.01036426919046959 # kj/(mol*Å) -> eV/Å
...
opts = {"comment":"diels a subsel",
        "architecture": [240,120,60,30,10,1],
        "lr": 1e-3,
        "subsel":[i for i in range(22)],
        "cutoff":torch.inf, # NOTE if this is changed recalc the sel
        "gamma": 0.9,
        "every": 40000,
        "batch_size": 1,
...
box_size = torch.tensor([25.81, 25.59, 25.16], device = device)
