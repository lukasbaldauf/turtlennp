...
def get_dm_dist_b(xyz, box_size):
    """
    Calculate the distance matrix and the distance tensor for batched atom coordinates.
    Args:
...
def get_local_frame_vectors(xyz, dm, dist, cutoff, sel, a_sel, atomtypes):
    ...
    B, N_, N = dist.size()
    descriptor_dists = torch.zeros((B, N_, sum(sel) + 3*sum(a_sel)), device=xyz.device)
    a_xyz = torch.zeros((B, N_, sum(a_sel), 3),  device=xyz.device, dtype=xyz.dtype)
    ...
    for j, selj in enumerate(sel):
        ...
        bega = sum(a_sel[:j])
        descriptor_dists[:, :, beg:beg + distance_block.shape[2]] = distance_block
        beg +=  sel[j] #distance_block.shape[2]
        if a_sel:
            a_idx = sorted_idx[:,:, :a_sel[j]].unsqueeze(-1).expand(-1,-1, -1, 3)
            a_xyz[:, :, bega:bega + a_sel[j]] = torch.gather(dm, 2, a_idx)
            a_dist[:, :, bega:bega + a_sel[j]] = sorted_dists[:, :, :a_sel[j]]
    # since we sort the distances according to closest atoms, we also
    # have to sort the s(x),s(y),s(z) according to closest atoms
    # does the order of scaling matter here? We ave to sort according to atomic indices
    ...
    if a_sel:
        ...
        if len(inf_vals[1])>0:
            raise NotImplementedError(f"Atoms {inf_vals[1]} has less than 2 neighbors!"
            "Can't construct local frame, but this might not be neccessary if this atom is not part of the subselection.",
        ...
        # Then scale by 1/rij
        a_xyz = a_xyz/(a_dist + epsilon)
        descriptor_dists[:,:,-sum(a_sel)*3:] = a_xyz.reshape(*descriptor_dists.shape[:2],sum(a_sel)*3)
...
class Model(nn.Module):
    def __init__(self,
        ...
        """
        TODO: how to treat transfereability? That is, can subsel, dynamic subsel
            change after training? Usually it wouldn't right?
        ...
        local atomic coordscalculate the subsel matrix of shape (len(subsel), N).
        ...
        self.norm = torch.zeros((len(self.sel), sum(self.sel) + 3*sum(self.a_sel), 2), device = device)
        self.norm[:,:, 1] = 1.0

        assert len(sel) == len(typemap), "Length of sel and typemap should be equal"
    ...
    def calculate_ef(
        ...
        descriptors = get_local_frame_vectors(xyz, dm, dist, self.cutoff, self.sel, self.a_sel, atomtypes)
        if descriptors_only:
            return descriptors

        # standardize the inputs
        for j in range(len(self.sel)):
            descriptors[atomtypes[:, subsel]==j] = (descriptors[atomtypes[:, subsel]==j] - self.norm[j,:,0])/self.norm[j, :, 1]
        ...
        # Compute forces
        if attach_grad:
            frci = -torch.autograd.grad(energy, xyz, create_graph=True, retain_graph=True, grad_outputs = torch.ones_like(energy))[0]
        else:
            frci = -torch.autograd.grad(energy, xyz, grad_outputs = torch.ones_like(energy))[0]
